...
{
	unsigned int tmp = readl((void __iomem *)addr);
	if(0) pr_debug("%s setmsk [%p] = 0x%08x\n", __func__, (void *)addr, tmp);
	mb();
...
{
	unsigned int tmp = readl((void __iomem *)addr);
	if(0) pr_debug("%s clrmsk [%p] = 0x%08x\n", __func__, (void *)addr, tmp);
	mb();
	writel((tmp & ~(msk)), (void __iomem *)addr);
	if(0) pr_debug("%s clrmsk [%p] = 0x%08x\n", __func__, (void *)addr, readl((void __iomem *)addr));
}
