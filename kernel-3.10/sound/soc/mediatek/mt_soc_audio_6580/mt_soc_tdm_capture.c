				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	PRINTK_AUD_UL1("AudDrv_MEMIF_Read finish0, read_count:%x, read_size:%x, u4DataRemained:%x,
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		PRINTK_AUD_UL1("AudDrv_MEMIF_Read finish1, copy size:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x,
		...
		PRINTK_AUD_UL1
		    ("AudDrv_MEMIF_Read finish2, copy size_1:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x,
		...
		PRINTK_AUD_UL1("AudDrv_MEMIF_Read finish3, copy size_2:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x,
