...
static int mtk_pcm_hdmi_start(struct snd_pcm_substream *substream)
{
	...
	pr_warn("[mtk_pcm_hdmi_start] AFE_IRQ_MCU_STATUS =0x%x AFE_IRQ_MCU_EN= 0x%x, AFE_IRQ_CNT5=0x%x,
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) {	/* copy once */
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				...
				PRINTK_AUD_HDMI2("[mtk_pcm_hdmi_copy]memcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx=0x%x,
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				...
				PRINTK_AUD_HDMI2("[mtk_pcm_hdmi_copy]mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx=%x,
