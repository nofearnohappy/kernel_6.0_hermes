...
kal_uint32 bq24160_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    /*battery_log(BAT_LOG_CRTI, "[bq24160_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24160_reg));*/
...
void bq24160_hw_component_detect(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24160_hw_component_detect] exist=%d, Reg[0x04]=0x%x\n",
        g_bq24160_hw_exist, val);
}

int is_bq24160_exist(void)
{
	battery_log(BAT_LOG_CRTI, "[is_bq24160_exist] g_bq24160_hw_exist=%d\n", g_bq24160_hw_exist);
...
{
    kal_uint8 i=0;
	battery_log(BAT_LOG_CRTI, "[bq24160] ");
    for (i=0;i<bq24160_REG_NUM;i++)
    {
        bq24160_read_byte(i, &bq24160_reg[i]);
		battery_log(BAT_LOG_CRTI, "[0x%x]=0x%x ", i, bq24160_reg[i]);
    }
