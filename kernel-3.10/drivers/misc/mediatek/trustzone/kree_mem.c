...
/* notice: handle type is the same */
	...
	KREE_SHAREDMEM_HANDLE *shm_handle, KREE_SHAREDMEM_PARAM *param)
{
	...
	if ((param->buffer >= (void *)PAGE_OFFSET) &&
		...
		pr_warn("[kree] KREE_RegisterSharedmem Error: support kmalloc only!!!\n");
...
#ifdef DBG_KREE_MEM
	pr_warn("KREE_UnreferenceSecuremem: count = 0x%x\n", count);
#endif
...
#ifdef DBG_KREE_MEM
		pr_warn("KREE_UnreferenceSecurechunkmem: count = 0x%x\n", count);
#endif
