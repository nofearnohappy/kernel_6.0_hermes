...
static struct la_context *la_context_obj = NULL;
...
static ssize_t la_show_enable_nodata(struct device* dev, 
...
/*----------------------------------------------------------------------------*/
static ssize_t la_show_active(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	struct la_context *cxt = NULL;
	int div = 0;
...
static ssize_t la_show_delay(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
    int len = 0;
	LA_LOG(" not support now\n");
...
static ssize_t la_show_sensordevnum(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	struct la_context *cxt = NULL;
...
static ssize_t la_show_batch(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 0); 
}
...
static ssize_t la_show_flush(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 0); 
}
...
int la_register_control_path(struct la_control_path *ctl)
{
	...
	err = sysfs_create_group(&la_context_obj->mdev.this_device->kobj,
			&la_attribute_group);
	if (err < 0)
	{
...
static int la_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&la_context_obj->idev->dev.kobj,
				&la_attribute_group);
	
	if((err = misc_deregister(&la_context_obj->mdev)))
	{
