...
#define MPLL_1333_FREQ          1333000
#define MPLL_800_FREQ            800000
...
static const struct freqhopping_ssc ssc_mpll_setting[] = {
	...
	/* 1 ~  0% (1333Mbps) Comment from Black.Chen, range is 1356Mbps~1333Mbps. */
	{MPLL_1333_FREQ, 0, 9, 1, 0, 0x1C000},
	/* 0 ~ -3% (800Mbps) Comment from Terry for 800MHz 0~-3% SCC. (Special Case) */
	{MPLL_800_FREQ, 0, 9, 0, 3, 0x1C000},
	{0, 0, 0, 0, 0, 0}
};
...
static void wait_mpll_dds_stable(unsigned int target_dds,
				 unsigned long reg_mon, unsigned int wait_count)
{
	...
	FH_MSG("[wait_mpll_dds_stable] time:%lu.%06lu count:%d dds:0x%x slope0:0x%x\n",
	       (unsigned long)now.tv_sec, (unsigned long)now.tv_nsec / NSEC_PER_USEC, i, fh_dds,
	       slope);
#else
	FH_MSG_DEBUG("mpll target_dds = 0x%x, fh_dds = 0x%x, i = %d\n", target_dds, fh_dds, i);
#endif
...
static int mt_fh_hal_dfs_armpll(unsigned int pll, unsigned int dds)
{
	...
	if (g_initialize == 0) {
		FH_MSG_DEBUG("(Warning) %s FHCTL isn't ready.", __func__);
		return -1;
	}
...
static int mt_fh_hal_dfs_mpll(unsigned int target_dds)
{
	...
	if (g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC) {
		/* /< default setting. (1600Mbps and 1488Mbps) */
		...
		fh_write32(g_reg_updnlmt[pll_id],
			   (PERCENT_TO_DDSLMT
			    ((fh_read32(g_reg_dds[pll_id]) & MASK21b),
			     p_setting->lowbnd) << 16) |
			   (PERCENT_TO_DDSLMT
			    ((fh_read32(g_reg_dds[pll_id]) & MASK21b), p_setting->upbnd)));
			       ...
			       void *data)
{
	...
	if (copy_from_user(dramc, buffer, len)) {
		FH_MSG_DEBUG("Error ret from %s %d", __func__, __LINE__);
		return 1;
	}
	...
	if (sscanf(dramc, "%d", &freq) == 1) {
		FH_MSG_DEBUG("%s %d", __func__, freq);
		if (1333 == freq) {
			/* low to hi */
			__fh_mpll_l2h();
		} else if (1226 == freq) {
		...
		FH_MSG_DEBUG("%s don't understand %s", __func__, dramc);
		return -EINVAL;
	}
}
			      ...
			      void *data)
{
	...
	p2 = p3 = p4 = p5 = 0;
	p1 = 9999; /* For fix Coverity issue (DEADCODE for switch default that cannot reach.)*/
