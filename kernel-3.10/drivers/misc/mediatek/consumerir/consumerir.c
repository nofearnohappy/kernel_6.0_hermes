...
//nfnh start
#define mt_consumerir_driver_name "consumerir"
static int consumerir_probe(struct platform_device *plat_dev)
{
	...
	if (!major) {
		error = alloc_chrdev_region(&dev_t_irtx, 0, 1, mt_consumerir_driver_name);
		if (!error) {
	...
	ret = register_chrdev_region(dev_t_irtx, 1, mt_consumerir_driver_name);
	if (ret) {
	...
	dev_class = class_create(THIS_MODULE, mt_consumerir_driver_name);
