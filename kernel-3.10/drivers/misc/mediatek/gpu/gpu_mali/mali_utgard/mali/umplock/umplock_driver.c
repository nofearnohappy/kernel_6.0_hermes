...
static int do_umplock_create_locked( _lock_cmd_priv *lock_cmd)
{
	...
	else if ( (i_index = umplock_find_item( lock_item->secure_id)) >= 0 )
	{
		...
		{
			pr_warn("UMPLOCK: whoops, item ran out of available reference slot\n" );
		}
	}
	else
	{
		...
		{
			pr_warn("UMPLOCK: whoops, ran out of available slots\n" );
		}
	}
...
static int do_umplock_process( _lock_cmd_priv *lock_cmd )
{
	...
	{
		/*fail to find a item*/
		pr_warn("UMPLOCK: IOCTL_UMPLOCK_PROCESS called with invalid parameter\n");
		mutex_unlock(&device.item_list_lock);
		return -EINVAL;
	}
	mutex_unlock(&device.item_list_lock);
...
static int do_umplock_release( _lock_cmd_priv *lock_cmd )
{
	...
	{
		/*fail to find item*/
		pr_warn("UMPLOCK: IOCTL_UMPLOCK_RELEASE called with invalid parameter\n");
		mutex_unlock(&device.item_list_lock);
		return -EINVAL;
	}
	mutex_unlock(&device.item_list_lock);
...
static int do_umplock_zap( void )
{
	...
	pr_warn("UMPLOCK: ZAP ALL ENTRIES!\n" );
...
static int do_umplock_dump( void )
{
	...
	for (i = 0; i < MAX_ITEMS; i++)
	{
		for (j = 0; j < MAX_PIDS; j++)
		{
			...
			{
				pr_warn("item[%d]->secure_id=%d\t reference[%d].ref_count=%d.pid=%d\n",
					i,
...
int do_umplock_client_add (_lock_cmd_priv *lock_cmd )
{
	...
	if( i==MAX_PIDS)
	{
		pr_warn("Oops, Run out of cient slots\n ");
	}
	return 0;
}
...
static int umplock_driver_open( struct inode *inode, struct file *filp )
{
	...
	pr_warn("UMPLOCK: OPEN SESSION (%i references)\n", atomic_read(&device.sessions) );
...
static int umplock_driver_release( struct inode *inode, struct file *filp )
{
	...
	pr_warn("UMPLOCK: CLOSE SESSION (%i references)\n", atomic_read(&device.sessions) );
...
{
	pr_warn("Inserting UMP lock device driver. Compiled: %s, time: %s\n", __DATE__, __TIME__ );

	if ( !umplock_constructor() )
	{
		pr_warn("UMP lock device driver init failed\n");
		return -ENOTTY;
	}

	pr_warn("UMP lock device driver loaded\n" );
...
{
	pr_warn("unloading UMP lock module\n" );
	umplock_destructor();
	pr_warn("UMP lock module unloaded\n" );
}
