...
static ssize_t yas_input_show_value(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    
	struct yas_state *st = i2c_get_clientdata(this_client);
	int16_t sensordata[4] = {0};
	int ret;
	//mutex_lock(&st->lock);
	ret = st->mag.ext(YAS537_GET_LAST_RAWDATA, sensordata);
	//mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
...
static ssize_t yas_input_show_op_mode(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t yas_input_store_op_mode(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct yas_state *st = i2c_get_clientdata(this_client);
	int ret;
	...
	//mutex_unlock(&st->lock);
        MAGN_LOG("ret for set_enable is %d\n", ret);
	return count;
}
...
static DRIVER_ATTR(cpsopmode,      0x1B6, yas_input_show_op_mode, yas_input_store_op_mode);
...
/*
	...
	ctl.m_enable = bmi160_m_enable;//yas_m_enable;
	ctl.m_set_delay  = bmi160_m_set_delay;//yas_m_set_delay;
	ctl.m_open_report_data = bmi160_m_open_report_data;//yas_m_open_report_data;
	ctl.o_enable = bmi160_o_enable;//yas_o_enable;
	ctl.o_set_delay  = bmi160_o_set_delay;//yas_o_set_delay;
	ctl.o_open_report_data = bmi160_o_open_report_data;//yas_o_open_report_data;
	...
	mag_data.get_data_o = bmi160_o_get_data;//yas_o_get_data;
	mag_data.get_data_m = bmi160_m_get_data;//yas_m_get_data;
