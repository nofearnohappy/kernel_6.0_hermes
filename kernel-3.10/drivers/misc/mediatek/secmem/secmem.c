...
static int secmem_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%s", cmd) == 1) {
		if (!strcmp(cmd, "1")) {
			...
			ret = secmem_api_alloc_pa(4096, 4096, &ref, &sec_handle);
			pr_err("[SECMEM] secmem_api_alloc_pa ret=%d sec_handle=%x\n", ret, sec_handle);
		} else if (!strcmp(cmd, "2")) {
			pr_debug("[SECMEM] - test for command 2\n");
			ret = secmem_api_unref_pa(sec_handle);
			pr_err("[SECMEM] secmem_api_unref_pa ret=%d\n", ret);
		}
	}
