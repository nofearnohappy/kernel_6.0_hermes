...
static int bmi160_step_d_set_delay(u64 ns)
{
	...
	STEP_C_FUN("bmi160_step_d_set_delay\n");
	if (value <= 5)
		sample_delay = BMI160_ACCEL_ODR_200HZ;
	else if (value <= 10)
		sample_delay = BMI160_ACCEL_ODR_100HZ;
		...
		sample_delay = BMI160_ACCEL_ODR_100HZ;
	STEP_C_LOG("bmi160_step_d_set_delay sensor delay value = %d, sample delay = %d\n",
			value, sample_delay);
	err = step_c_set_datarate(sample_delay);
	if (err < 0)
		STEP_C_ERR("bmi160_step_d_set_delay set data rate error.\n");
	return err;
}
