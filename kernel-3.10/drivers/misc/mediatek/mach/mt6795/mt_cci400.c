...
static struct platform_driver mt_cci400_drv = {
	.driver = {
	.name = "cci400",
...
static ssize_t enable_spec_fetch_show(struct device_driver *driver, char *buf)
{
	...
	if (val & (0x1 << 2))
		ptr += sprintf(ptr, "[CCI] Disable speculative fetches in CTRL Override Re:0x%x\n", val);
	else
		ptr += sprintf(ptr, "[CCI] Enable speculative fetches in CTRL Override Reg:0x%x\n", val);
...
int mt_cci400_init(void){
		...
		if (ret == 0)
			pr_err("[CCI] CCI 400 driver init done.\n");
#if 0
