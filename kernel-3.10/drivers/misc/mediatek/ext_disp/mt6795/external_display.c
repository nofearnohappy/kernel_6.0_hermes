...
{
	EXT_DISP_ERR("extd cmdq timeout:%llu\n", engineFlag);
	ext_disp_diagnose();
	return 0;
}
...
int ext_disp_init(char *lcm_name, unsigned int session)
{
	...
	if (ext_disp_mode == EXTD_DIRECT_LINK_MODE) {
		...
		EXT_DISP_LOG("ext_disp display is DEBUG RDMA to dpi MODE\n");
	} else {
	...
	disp_ddp_path_config *data_config = vmalloc(sizeof(disp_ddp_path_config));
	if (data_config) {
		...
		EXT_DISP_LOG("allocate buffer data_config failed!!!\n");
		ret = EXT_DISP_STATUS_ERROR;
		goto done;
	}
