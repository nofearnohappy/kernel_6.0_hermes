...
//************************************
extern unsigned int get_cur_gpu_freq_wapper(void);
...
static struct cpu_index_st cpu_index_list[NUMBER_OF_CORE];
static int cpufreqs[NUMBER_OF_CORE];
static int cpuloadings[NUMBER_OF_CORE];
    ...
    int **gpuloading)
{
    ...
    if (gpufreq)
    {
        gpufreqs[0] = get_cur_gpu_freq_wapper();
        *gpufreq = gpufreqs;
    }
