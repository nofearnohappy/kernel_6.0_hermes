...
void vWriteCore1Reg(UINT32 dAddr, UINT32 dVal)
{
	ULONG VA_CORE1_BASE = 0xF6030000;
	mt_reg_sync_writel(dVal, (ULONG)(VA_CORE1_BASE + dAddr));
}

UINT32 u4ReadCore1Reg(UINT32 dAddr)
{
	ULONG VA_CORE1_BASE = 0xF6030000;
	return (UINT32)(*(volatile UINT32 *)(VA_CORE1_BASE + dAddr));
}


void vWriteLaeReg(UINT32 dAddr, UINT32 dVal)
{
	ULONG VA_LAE_BASE = 0xF6010000;
	mt_reg_sync_writel(dVal, (ULONG)(VA_LAE_BASE + dAddr));
}

UINT32 u4ReadLaeReg(UINT32 dAddr)
{
	ULONG VA_LAE_BASE = 0xF6010000;
	return (UINT32)(*(volatile UINT32 *)(VA_LAE_BASE + dAddr));
}
...
void vVDecWriteUFO(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    vWriteGconReg(UFO_REG_OFFSET0 + u4Addr, u4Val);
}


UINT32 u4VDecReadUFO(UINT32 u4VDecID, UINT32 u4Addr)
{
    UINT32 u4Val;
    u4Val = (u4ReadGconReg(UFO_REG_OFFSET0 + u4Addr));
    return u4Val;
}
...
#endif
{
    ...
    switch (u4VDecType)
    {
        ...
        case VDEC_S263:
            ...
            {
                printk("Rocky UFO reset\n");
                vVDecWriteUFO(u4VDecID,36*4,1);
                vVDecWriteUFO(u4VDecID,36*4,0);
            }
        ...
        case VDEC_VC1:
            ...
            u4VDecPDNCtrlSpec = 0xFB;
            u4VDecPDNCtrlModule = 0x3E6A1108;
#if (VDEC_UFO_SUPPORT && (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_ROCKY))
            printk("Rocky UFO reset\n");
            vVDecWriteUFO(u4VDecID,36*4,1);
            vVDecWriteUFO(u4VDecID,36*4,0);
#endif
    ...
    if (((u4Val >> 16) & 0x1))
    {
        ...
        vVDecWriteMISC(u4VDecID, 0x210, addr_offset);   //MISC_132fake engine start address offset (size:256*16 align)
        //vVDecWriteMISC(u4VDecID, 0x214, 0x00100010);    //MISC_133[15:0]= 16; MISC_133[31:16]= 16,       16 requests & sleep 16 cycles
        //vVDecWriteMISC(u4VDecID, 0x214, 0x01000010);    //MISC_133[15:0]= 16; MISC_133[31:16]= 256,      16 requests & sleep 256 cycles
        vVDecWriteMISC(u4VDecID, 0x214, 0x01000008);    //MISC_133[15:0]= 8; MISC_133[31:16]= 256,      8 requests & sleep 256 cycles
        vVDecWriteMISC(u4VDecID, 0x218, 0x10001);       //MISC_134[0]= 0x10001 ([0]alw_rd, [16]fake start)
