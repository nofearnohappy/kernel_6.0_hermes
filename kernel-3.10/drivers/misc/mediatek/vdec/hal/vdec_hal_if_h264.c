...
INT32 i4VDEC_HAL_H264_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_H264_INIT_PRM_T *prH264VDecInitPrm)
{
    ...
    vVDecAVCResetHW(u4VDecID, VDEC_H264);
#else
...
INT32 i4VDEC_HAL_H264_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_H264_BS_INIT_PRM_T *prH264BSInitPrm)
{
    ...
    if (u4BSID == 0)
    {
        ...
        vVDecWriteAVCVLD(u4VDecID, RO_AVLD_TIMEOUT_THD, AVLD_TIMEOUT_THD);
#endif
        fgInitBSResult = fgInitH264BarrelShift1(u4BSID, u4VDecID, prH264BSInitPrm);
    }
    else
    {
        fgInitBSResult = fgInitH264BarrelShift2(u4BSID, u4VDecID, prH264BSInitPrm);
    }
...
void vVDEC_HAL_H264_WriteScalingList(UINT32 u4VDecID, UINT32 u4Idx, CHAR *pcSlicePtr)
{
    ...
    if (u4Idx < 6)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            vVDecWriteAVCCOMVLD(u4VDecID, RW_VLD_SCL_ADDR, 0x200 + u4Idx + (i << 2));
            vVDecWriteAVCCOMVLD(u4VDecID, RW_VLD_SCL_DATA, u4Temp);
            vVDecWriteAVCCOMVLD(u4VDecID, RW_VLD_SCL_ADDR, 0x100 + u4Idx + (i << 2));
        }
    }
    else
    {
...
void vVDEC_HAL_H264_PredWeightTable(UINT32 u4VDecID)
{
    ...
    u4Data = u4VDecReadAVCMISC(u4VDecID, 60 * 4);
    u4Data |= 0x01;
    vVDecWriteAVCMISC(u4VDecID, 60 * 4, u4Data);
    // end if MT6589
...
void vVDEC_HAL_H264_InitPRefList(UINT32 u4VDecID, BOOL fgPicFrm, UINT32 u4MaxFrameNum, UINT32 u4CurrPicNum)
{
    ...
    if (bEnable_UFO[u4VDecID]) {
        ...
        vVDecWriteAVCMC(u4VDecID, 664 * 4, 0x10);
        vVDecWriteAVCMC(u4VDecID, 722 * 4, 0x01);
    }

    vVDecWriteAVCMC(u4VDecID, RW_AMC_P_LIST0_FLD, 0);
    ...
    if (bMode_MCORE[u4VDecID]) {
        UCHAR ucRegIdx;
        for (ucRegIdx = 0; ucRegIdx < 32; ++ ucRegIdx)
        {
#if CONFIG_DRV_VIRTUAL_ADDR
            vVDecWriteAVCMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, (728 * 4) + (ucRegIdx << 2), PHYSICAL((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
#else
            vVDecWriteAVCMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, (728 * 4) + (ucRegIdx << 2), u4AbsDramANc((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
#endif
        }
    }
}
...
void vVDEC_HAL_H264_SetPRefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_P_REF_PRM_T *prPRefPicListInfo)
{
    ...
    if (bEnable_UFO[u4VDecID]) {
        ...
        {
            UINT32 u4RegValue;
            u4RegValue = u4VDecReadAVCMC(u4VDecID, 664 * 4);
            printk("MC_664 read == 0x%08x\n", u4RegValue);
            vVDecWriteAVCMC(u4VDecID, 664 * 4, u4RegValue | 0x010);
            printk("MC_664 write == 0x%08x\n", u4RegValue | 0x010);
            vVDecWriteAVCMC(u4VDecID, 722 * 4, 0x01);
            u4RegValue = u4VDecReadAVCMC(u4VDecID, 718 * 4);
            printk("MC_718 read == 0x%08x\n", u4RegValue);
            vVDecWriteAVCMC(u4VDecID, 718 * 4, u4RegValue | (0x01 << ucRegIdx));
    ...
    if (ucFld == FRAME)
    {
        //bRegIdx = bRefPicIdx;
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteAVCMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG)  prPRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteAVCMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG)  prPRefPicListInfo->u4FBufYStartAddr));
#endif
#if VDEC_MVC_SUPPORT
    ...
    else
    {
        ...
        else
        {
            ...
            vVDecWriteAVCMC(u4VDecID, RW_AMC_P_LIST0_FLD, u4VDecReadAVCMC(u4VDecID, RW_AMC_P_LIST0_FLD) | (0x1 << ucRegIdx));
        }
    }
}
...
{
    vVDecWriteAVCMC(u4VDecID, RW_AMC_B_LIST0_FLD, 0);
    vVDecWriteAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD, 0);
    if (bMode_MCORE[u4VDecID]) {
        UCHAR ucRegIdx;
        for (ucRegIdx = 0; ucRegIdx < 32; ++ ucRegIdx)
        {
#if CONFIG_DRV_VIRTUAL_ADDR
            vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), PHYSICAL((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, (760 * 4) + (ucRegIdx << 2), PHYSICAL((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, (792 * 4) + (ucRegIdx << 2), PHYSICAL((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
#else
            vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), u4AbsDramANc((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, (760 * 4) + (ucRegIdx << 2), u4AbsDramANc((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
            vVDecWriteAVCMC(u4VDecID, (792 * 4) + (ucRegIdx << 2), u4AbsDramANc((ULONG)_ptFBufInfo[u4VDecID][0].u4Addr));
#endif
        }
    }
}
...
BOOL bVDEC_HAL_H264_SetBRefRegforUFO(UINT32 u4VDecID, VDEC_INFO_H264_B_REF_PRM_T *prBRefPicListInfo, BOOL bForB0, BOOL bForB1)
{
    ...
    if (bB0B1Both == TRUE)
    {
        if (prBRefPicListInfo->ucPicStruct == FRAME)
        {
            ...
            printk("MC_664 write == 0x%08x\n", u4RegValue | 0x010);
            vVDecWriteAVCMC(u4VDecID, 722 * 4, 0x01);
            u4RegValue = u4VDecReadAVCMC(u4VDecID, 719 * 4);
            printk("MC_719 read == 0x%08x\n", u4RegValue);
            vVDecWriteAVCMC(u4VDecID, 719 * 4, u4RegValue | (0x01 << ucRegIdx));
        ...
        if (prBRefPicListInfo->ucPicStruct1 == FRAME)
        {
            ...
            printk("MC_664 write == 0x%08x\n", u4RegValue | 0x010);
            vVDecWriteAVCMC(u4VDecID, 722 * 4, 0x01);
            u4RegValue = u4VDecReadAVCMC(u4VDecID, 720 * 4);
            printk("MC_720 read == 0x%08x\n", u4RegValue);
            vVDecWriteAVCMC(u4VDecID, 720 * 4, u4RegValue | (0x01 << ucRegIdx));
...
BOOL bVDEC_HAL_H264_SetBRefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_B_REF_PRM_T *prBRefPicListInfo)
{
    ...
    if (ucFld == FRAME)
    {
        // B_0
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#endif
...
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), PHYSICAL((ULONG) prBRefPicListInfo->u4FBufYStartAddr1));
#else
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufYStartAddr1));
#endif
    ...
    else
    {
        ...
        if ((prBRefPicListInfo->u4ListIdx == 0) || (prBRefPicListInfo->u4ListIdx == 1) || (prBRefPicListInfo->u4ListIdx == 4) || (prBRefPicListInfo->u4ListIdx == 5))
        {
            ...
            else
            {
                ...
                vVDecWriteAVCMC(u4VDecID, RW_AMC_B_LIST0_FLD, u4VDecReadAVCMC(u4VDecID, RW_AMC_B_LIST0_FLD) | (0x1 << ucRegIdx));
            }
            if (bEnable_UFO[u4VDecID]) {
        ...
        if ((prBRefPicListInfo->u4ListIdx == 2) || (prBRefPicListInfo->u4ListIdx == 3) || (prBRefPicListInfo->u4ListIdx == 4) || (prBRefPicListInfo->u4ListIdx == 5))
        {
            ...
            {
#if CONFIG_DRV_VIRTUAL_ADDR
                vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), PHYSICAL((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#else
                vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#endif
            ...
            else
            {
                ...
                vVDecWriteAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD, u4VDecReadAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD) | (0x1 << ucRegIdx));
            }
            if (bEnable_UFO[u4VDecID]) {
...
void vVDEC_HAL_H264_B1ListSwap(UINT32 u4VDecID, BOOL fgIsFrmPic)
{
    ...
    if (fgIsFrmPic)
    {
        u4Temp = u4VDecReadAVCMC(u4VDecID, RW_MC_B_LIST1);
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1, u4VDecReadAVCMC(u4VDecID, RW_MC_B_LIST1 + 4));
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + 4, u4Temp);
    ...
    else
    {
        ...
        u4Temp = u4VDecReadAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD) & 1; // bit 0
        // Write bit 1 to bit 0
        vVDecWriteAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD, (u4VDecReadAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD) & (~1)) | ((u4VDecReadAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD)  >> 1) & 1));
        vVDecWriteAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD, (u4VDecReadAVCMC(u4VDecID, RW_AMC_B_LIST1_FLD) & (~2)) | (u4Temp << 1));
    }
    if (bEnable_UFO[u4VDecID]) {
        UINT u4Bit0, u4Bit1;
        u4Temp = u4VDecReadAVCMC(u4VDecID, 720 * 4);
        u4Bit0 = (u4Temp & 0x00000001) ? 0x01 : 0x00;
        u4Bit1 = (u4Temp & 0x00000002) ? 0x01 : 0x00;
        vVDecWriteAVCMC(u4VDecID, 720 * 4, (u4Temp & 0xFFFFFFFC) | (u4Bit0 << 1) | (u4Bit1));
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    vVDecWriteAVCVLDTOP(u4VDecID, RW_VLD_PIC_MB_SIZE_M1, ((prSPS->u4PicHeightInMapUnitsMinus1) << VLD_TOP_PIC_HEIGHT_IN_MBS_POS) | prSPS->u4PicWidthInMbsMinus1);
#else
...
#endif
    if (bMode_10bit[u4VDecID]) {
    	vVDecWriteAVCVLDTOP(u4VDecID, RW_VLD_TOP_BIT_DEPTH, ((prSPS->u4BitDepthLumaMinus8 & 0xf) << 0 ) |((prSPS->u4BitDepthChromaMinus8 & 0xf) << 4 ));
	    if (prSPS->u4BitDepthLumaMinus8 == 2) { // only Y bitdepth=10 in mono videos
            if ( !bMode_MCORE[u4VDecID] || _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4CoreID != AVC_LAE_0_ID )
            {
		        vVDecWriteAVCMC(u4VDecID, 827*4, 0x1);
    		    vVDecWriteAVCPP(u4VDecID, 739*4, 0x1); //set compact mode
            }
    	}
    }
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_ROCKY)
    if (bEnable_UFO[u4VDecID]) {
        vVDecWriteAVCUFO(u4VDecID, 35 * 4, (u4VDecReadAVCUFO(u4VDecID, 35 * 4) & ~0xf0) | ((prSPS->u4BitDepthLumaMinus8 & 0xf) << 4));
    }
#endif
}
...
void vVDEC_HAL_H264_SetRegBeforeTrigDecode(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    UINT32 u4SHDRInfo;
    UINT32 u4ResultReg;
    ...
    vVDecWriteAVCCRC(u4VDecID, 0x4, u4CRCSrc);
#endif
    ...
    // addr swap mode
    if (!bMode_FieldCompact[u4VDecID]) {
        vVDecWriteAVCMC(u4VDecID, RW_MC_ADDRSWAP, prDecPrm->ucAddrSwapMode);
    }

#if (CONFIG_DRV_VERIFY_SUPPORT)
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    vVDecWriteAVCVLDTOP(u4VDecID, RW_VLD_TOP_BUSY_THRESHOLD, 0xFFFFFFFF);
#else
    ...
    vVDecWriteAVCMC(u4VDecID, RW_MC_UMV_PIC_WIDTH, prDecPrm->u4PicW);
    vVDecWriteAVCMC(u4VDecID, RW_MC_UMV_PIC_HEIGHT, prDecPrm->u4PicH);
    ...
    {
        vVDecWriteAVCMC(u4VDecID, RW_MC_WRAPPER_SWITCH, 1);
    }
    else
    {
        vVDecWriteAVCMC(u4VDecID, RW_MC_WRAPPER_SWITCH, 0);
    }
#endif
    ...
    {
        //test only
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_ENABLE, 0);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_WB_BY_POST, 0);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_DBLK_MODE, 0);
    }
    else
    {
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_ENABLE, 1);
        if (bEnable_UFO[u4VDecID]) {
            UINT32 u4PIC_W_H = ((((prDecPrm->u4PicW + 15) >> 4) - 1) << 16) | (((prDecPrm->u4PicH + 15) >> 4) - 1);
            vVDecWriteAVCMC(u4VDecID, 700 * 4, u4PIC_W_H);
            ...
            if (fgIsFrmPic(u4VDecID))
            {
                u4RegValue = u4VDecReadAVCMC(u4VDecID, 664 * 4);
                printk("MC_664 read == 0x%08x\n", u4RegValue);
                vVDecWriteAVCMC(u4VDecID, 664 * 4, u4RegValue | 0x01);
                printk("MC_664 write == 0x%08x\n", u4RegValue | 0x01);
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_ROCKY)
                vVDecWriteAVCUFO(u4VDecID, 35 * 4, (u4VDecReadAVCUFO(u4VDecID, 35 * 4) | 0x1));
#endif
                memset(prH264DecPrm->prCurrFBufInfo->u4YStartAddr, 0, prH264DecPrm->prCurrFBufInfo->u4DramPicArea);
            }
            // for pp out (UFO ENC)
            vVDecWriteAVCMC(u4VDecID, 698 * 4, PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS);
            vVDecWriteAVCMC(u4VDecID, 699 * 4, PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y);
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_ROCKY)
            vVDecWriteAVCUFO(u4VDecID, 31 * 4, PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS);
            vVDecWriteAVCUFO(u4VDecID, 32 * 4, PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y);
#endif
            // for MC (UFO DEC)
            vVDecWriteAVCMC(u4VDecID, 663 * 4, prDecPrm->u4PIC_SIZE_BS);
            vVDecWriteAVCMC(u4VDecID, 701 * 4, prDecPrm->u4UFO_LEN_SIZE_Y);
            vVDecWriteAVCMC(u4VDecID, 343 * 4, prDecPrm->u4PIC_SIZE_Y_BS);
        } else {
            //vVDecWriteMC(u4VDecID, 664 * 4, 0);
            //vVDecWriteMC(u4VDecID, 722 * 4, 0);
        }
        if (bEnable_DDR4[u4VDecID]) {
            if (prDecPrm->ucPicStruct == FRAME) {
                vVDecWriteAVCPP(u4VDecID, 744 * 4, (u4VDecReadAVCPP(u4VDecID, 744 * 4) | 0x1));
                vVDecWriteAVCPP(u4VDecID, 814 * 4, (u4VDecReadAVCPP(u4VDecID, 814 * 4) | 0x1));
                UINT32 Reg_PP_value1, Reg_PP_value2;
                if (bEnable_UFO[u4VDecID]) {
                    Reg_PP_value1 = u4VDecReadAVCPP(u4VDecID, 828 * 4);
                    Reg_PP_value2 = u4VDecReadAVCPP(u4VDecID, 829 * 4);
                } else {
                    Reg_PP_value1 = u4VDecReadAVCPP(u4VDecID, 749 * 4);
                    Reg_PP_value2 = u4VDecReadAVCPP(u4VDecID, 750 * 4);
                }
                printk("before DDR4 register value = 0x%x 0x%x\n", Reg_PP_value1, Reg_PP_value2);
            } else {
                vVDecWriteAVCPP(u4VDecID, 744 * 4, (u4VDecReadAVCPP(u4VDecID, 744 * 4) & (~0x1)));
                vVDecWriteAVCPP(u4VDecID, 814 * 4, (u4VDecReadAVCPP(u4VDecID, 814 * 4) & (~0x1)));
            }
...
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_Y_ADDR, PHYSICAL((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) >> 9);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_C_ADDR, PHYSICAL((ULONG)(prH264DecPrm->prCurrFBufInfo->u4YStartAddr + prH264DecPrm->prCurrFBufInfo->u4CAddrOffset)) >> 8);
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_ROCKY)
        if (bEnable_UFO[u4VDecID]) {
            vVDecWriteAVCUFO(u4VDecID, 33 * 4, PHYSICAL((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) >> 9);
            vVDecWriteAVCUFO(u4VDecID, 34 * 4, PHYSICAL((ULONG)(prH264DecPrm->prCurrFBufInfo->u4YStartAddr + prH264DecPrm->prCurrFBufInfo->u4CAddrOffset)) >> 8);
        }
#endif
#else
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_Y_ADDR, u4AbsDramANc((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) >> 9);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_C_ADDR, u4AbsDramANc((ULONG)(prH264DecPrm->prCurrFBufInfo->u4YStartAddr + prH264DecPrm->prCurrFBufInfo->u4CAddrOffset)) >> 8);
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_ROCKY)
        if (bEnable_UFO[u4VDecID]) {
            vVDecWriteAVCUFO(u4VDecID, 33 * 4, u4AbsDramANc((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) >> 9);
            vVDecWriteAVCUFO(u4VDecID, 34 * 4, u4AbsDramANc((ULONG)(prH264DecPrm->prCurrFBufInfo->u4YStartAddr + prH264DecPrm->prCurrFBufInfo->u4CAddrOffset)) >> 8);
        }
#endif
#endif
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_WB_BY_POST, 1);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_DBLK_MODE, DBLK_Y | DBLK_C);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_X_RANGE, ((prDecPrm->u4PicW + 15) >> 4) - 1);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_Y_RANGE, (((prDecPrm->u4PicH >> (1 - (prH264DecPrm->fgIsFrmPic))) + 15) >> 4) - 1);
        vVDecWriteAVCMC(u4VDecID, RW_MC_PP_MB_WIDTH, ((prDecPrm->u4PicW + 15) >> 4));
    }

#if (CONFIG_DRV_FPGA_BOARD)
	vVDecWriteAVCMC(u4VDecID, RW_MC_MODE_CTL, MC_QIU_BANK4 | MC_QIU_BANK8 | MC_DRAM_REQ_DELAY_1T | MC_DRAM_REQ_MERGE_OFF | MC_MV_MERGE_OFF);
#endif

#if CONFIG_DRV_VIRTUAL_ADDR
	vVDecWriteAVCMC(u4VDecID, RW_AMC_Y_OUT_ADDR, PHYSICAL((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr));
#else
	vVDecWriteAVCMC(u4VDecID, RW_AMC_Y_OUT_ADDR, u4AbsDramANc((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr));
#endif
	vVDecWriteAVCMC(u4VDecID, RW_AMC_CBCR_OFFSET, prH264DecPrm->prCurrFBufInfo->u4CAddrOffset);

#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    vVDecWriteAVCMC(u4VDecID, RW_MC_PIC_W_MB, ((prDecPrm->u4PicBW + 15) >> 4));
#else
    vVDecWriteAVCCOMVLD(u4VDecID, RW_VLD_PIC_W_MB, ((prDecPrm->u4PicBW + 15) >> 4));
#endif
    ...
    if ( bMode_MCORE[u4VDecID] && _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4CoreID != AVC_LAE_0_ID ){
        vVDecWriteAVCMV(u4VDecID, 136*4, u4VDecReadAVCMV(u4VDecID, 136*4) | 0x1 << 12);
        vVDecWriteAVCMV(u4VDecID, 136*4, u4VDecReadAVCMV(u4VDecID, 136*4) | 0x1 << 14);
    }
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
    vVDecWriteAVCMC(0, 0x5E4, (u4VDecReadAVCMC(0, 0x5E4) | (0x1 << 12)));

#if (!VDEC_8320_SUPPORT)
    vVDecWriteAVCMC(0, 0x660, (u4VDecReadAVCMC(0, 0x660) | (0x80000000)));
#endif

#ifndef VDEC_PIP_WITH_ONE_HW
    vVDecWriteAVCMC(1, 0x5E4, (u4VDecReadAVCMC(1, 0x5E4) | (0x1 << 12)));
#if (!VDEC_8320_SUPPORT)
    vVDecWriteAVCMC(1, 0x660, (u4VDecReadAVCMC(1, 0x660) | (0x80000000)));
#endif
#endif
#endif
    ...
    if (bMode_FieldCompact[u4VDecID]) {
        ...
        vVDecWriteAVCPP(u4VDecID, 0x3C, (u4VDecReadAVCPP(u4VDecID, 0x3c) & 0xEFFFFFFF));
        vVDecWriteAVCMC(u4VDecID, 0x920, (u4VDecReadAVCMC(u4VDecID, 0x920) & 0xFEFFFFFF));
    } else {
        vVDecWriteAVCMC(u4VDecID, 0x920, (u4VDecReadAVCMC(u4VDecID, 0x920) | 0x01000000));
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8550)
        vVDecWriteAVCMC(u4VDecID, RW_MC_NBM_CTRL,
                     ((u4VDecReadAVCMC(u4VDecID, RW_MC_NBM_CTRL)  & 0xFFFFFFF8) | prDecPrm->ucAddrSwapMode));
#endif
    }

    // turn off test mode 0x834[4]
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8560)
    vVDecWriteAVCMC(u4VDecID, RW_MC_DDR3_EN, (u4VDecReadAVCMC(u4VDecID, RW_MC_DDR3_EN)  & 0xFFFFFFEF));
#endif

    if (prDecPrm->u4PicBW < 176)
    {
        vVDecWriteAVCMISC(u4VDecID, VDEC_DV_LAT_BUF_BYPASS, u4VDecReadDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS) | 0x1);
#if 1  //for 6582
    ...
    {
        vVDecWriteAVCMISC(u4VDecID, VDEC_DV_LAT_BUF_BYPASS, u4VDecReadDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS) & 0xFFFFFFFE);
    }


#if VDEC_DDR3_SUPPORT
    u4DDR3_PicWdith = (((prDecPrm->u4PicBW + 63) >> 6) << 2);
    vVDecWriteAVCMC(u4VDecID, RW_MC_PP_MB_WIDTH, u4DDR3_PicWdith);
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    vVDecWriteAVCMC(u4VDecID, RW_MC_PIC_W_MB, u4DDR3_PicWdith);
#else
    vVDecWriteAVCCOMVLD(u4VDecID, RW_VLD_PIC_W_MB, u4DDR3_PicWdith);
#endif

    //Turn on DDR3 mode
    vVDecWriteAVCMC(u4VDecID, RW_MC_DDR_CTRL0,
                 ((u4VDecReadAVCMC(u4VDecID, RW_MC_DDR_CTRL0)  & 0xFFFFFFFE) | 0x1));

	vVDecWriteAVCMC(u4VDecID, RW_MC_DDR_CTRL1,
    			 ((u4VDecReadAVCMC(u4VDecID, RW_MC_DDR_CTRL1)  & 0xFFFFFFFE) | 0x1));

    aMc406 = u4VDecReadAVCMC(u4VDecID, (406 << 2));
    aMc406 &= 0xFFFFFFEF;
    vVDecWriteAVCMC(u4VDecID, (406 << 2), aMc406);

    //Turn-on DDR3, Set 0x834[0] = 0
    vVDecWriteAVCMC(u4VDecID, RW_MC_DDR3_EN, (u4VDecReadAVCMC(u4VDecID, RW_MC_DDR3_EN)  & 0xFFFFFFFE));

    //Post-process enable
    vVDecWriteAVCMC(u4VDecID, RW_MC_PP_ENABLE, (u4VDecReadAVCMC(u4VDecID, RW_MC_PP_ENABLE)  | 0x1));

    //Writeback by PP
    vVDecWriteAVCMC(u4VDecID, RW_MC_PP_WB_BY_POST, 0x00000001);
#endif
    ...
    vVDecWriteAVCVLDTOP(u4VDecID, RW_VLD_TOP_TOTAL_MBS_IN_PIC, u4total_mbs_in_pic);
    vVDecWriteAVCVLDTOP(u4VDecID, RO_VLD_TOP_TIMEOUT_THD, VLD_TOP_TIMEOUT_THD);
#endif
    ...
    vVDecWriteAVCMC(u4VDecID, RW_MC_VLD_WRAPPER, PHYSICAL(_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4VLDWrapperWrok));
    //vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER, 0x00200000);
    //DBG_H264_PRINTF("MC_648 = 0x%08x\n", 0x00200000);
#if MT6582_L2_EMULATION == 1
    vVDecWriteAVCMC(u4VDecID, RW_MC_PP_WRAPPER, PHYSICAL(_pucPredSa[u4VDecID]) + 0x5A00);
#elif MT6582_L2_EMULATION == 2
    vVDecWriteAVCMC(u4VDecID, RW_MC_PP_WRAPPER, 0x00205A00);
#else
    vVDecWriteAVCMC(u4VDecID, RW_MC_PP_WRAPPER, PHYSICAL(_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4PPWrapperWrok));
#endif
    ...
    //pp_reg_513[4] = 1'b0
    u4ResultReg = u4VDecReadAVCPP(u4VDecID, 0x804);
    u4ResultReg &= 0xFFFFFFEF;
    vVDecWriteAVCPP(u4VDecID, 0x804 , u4ResultReg);
    ...
    u4ResultReg = u4VDecReadAVCVLDTOP(u4VDecID, 0x68);
    ...
    u4ResultReg = u4VDecReadAVCVLDTOP(u4VDecID, 0x28);
    ...
    vVDecWriteAVCVLDTOP(u4VDecID, 0x94, u4PredRdDramEnd);
    ...
    if (prDecPrm->u4PicBW <= 176)
    {
        ...
        u4ResultReg = u4VDecReadAVCVLDTOP(u4VDecID, 0x2C);
        u4ResultReg |= 0x40000000;
        vVDecWriteAVCVLDTOP(u4VDecID, 0x2C , u4ResultReg);

#if 1
        //Vld_top_16[5:4] = 2'b2 // WCP2 dram burst mode
        u4ResultReg = u4VDecReadAVCVLDTOP(u4VDecID, 0x40);
        u4ResultReg &= 0xFFFFFFEF;
        u4ResultReg |= 0x00000020;
        vVDecWriteAVCVLDTOP(u4VDecID, 0x40 , u4ResultReg);
#else
    ...
    vVDecWriteAVCMISC(u4VDecID, VDEC_CRC_REG_EN, 1<<4);
    vVDecWriteAVCVLDTOP(u4VDecID, RW_VLD_TOP_PIC_PIX_SIZE, ((((prDecPrm->u4PicW) & 0xffff) << 0 ) | (((prDecPrm->u4PicH) & 0xffff) << 16 )));
    if (bMode_FieldCompact[u4VDecID]) {
        printk("MC_36 read == 0x%08x\n",  u4VDecReadAVCMC(u4VDecID, 36 * 4));
        printk("MC_474 read == 0x%08x\n", u4VDecReadAVCMC(u4VDecID, 474 * 4));
        printk("MC_584 read == 0x%08x\n", u4VDecReadAVCMC(u4VDecID, 584 * 4));
        printk("PP_15 read == 0x%08x\n",  u4VDecReadAVCPP(u4VDecID, 15 * 4));
    }
    if (bEnable_UFO[u4VDecID]) {
        printk("MC_664 read == 0x%08x\n", u4VDecReadAVCMC(u4VDecID, 664 * 4));
        printk("MC_718 read == 0x%08x\n", u4VDecReadAVCMC(u4VDecID, 718 * 4));
        printk("MC_719 read == 0x%08x\n", u4VDecReadAVCMC(u4VDecID, 719 * 4));
        printk("MC_720 read == 0x%08x\n", u4VDecReadAVCMC(u4VDecID, 720 * 4));
        printk("MC_722 read == 0x%08x\n", u4VDecReadAVCMC(u4VDecID, 722 * 4));
        printk("UFO_35 read == 0x%08x\n", u4VDecReadAVCUFO(u4VDecID, 35 * 4));
        printk("UFO_31 read == 0x%08x\n", u4VDecReadAVCUFO(u4VDecID, 31 * 4));
        printk("UFO_32 read == 0x%08x\n", u4VDecReadAVCUFO(u4VDecID, 32 * 4));
        printk("UFO_33 read == 0x%08x\n", u4VDecReadAVCUFO(u4VDecID, 33 * 4));
        printk("UFO_34 read == 0x%08x\n", u4VDecReadAVCUFO(u4VDecID, 34 * 4));
    }
}
...
{
    if (bMode_MCORE[u4VDecID]) {
        vVDecAVCSetCoreState(u4VDecID, AVC_LAE_0_ID);
        vVDEC_HAL_H264_SetRegBeforeTrigDecode(u4VDecID, prDecPrm);
        vVDecAVCSetCoreState(u4VDecID, AVC_CORE_0_ID);
        vVDEC_HAL_H264_SetRegBeforeTrigDecode(u4VDecID, prDecPrm);

        vVDecAVCSetCoreState(u4VDecID, AVC_LAE_0_ID);
        printk("[INFO] AVC_LAE_0_ID input Window: 0x%08X\n", u4VDecReadAVCVLD(u4VDecID, RO_AVLD_BARL));

        //vVDEC_HAL_H264_VDec_DumpReg_LAE(u4VDecID);
    
        printk("Before decoding AVC LAE finish flag AVCMCORE_TOP_3[24] %d\n", (u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) >> 24));
        // trigger LAE decode
        vVDecWriteAVCVLD(u4VDecID, RW_AVLD_PROC, 0x1);
	    vVDEC_HAL_H264_Wait_LAE_decode_finished(u4VDecID);
        printk("After decoding AVC LAE finish flag AVCMCORE_TOP_3[24] %d\n", (u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) >> 24));

        // clear LAE interrupt
        u4VDEC_HAL_H264_VDec_ClearInt(u4VDecID);
#if 0
        // dump LAE buffer
        if ( _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4CoreID == AVC_LAE_0_ID ) {
            fgWrData2PC((void *)(_pucLAEBuffer[u4VDecID][0]), 1152*(2304/16), 7, "/mnt/sdcard1/LAE_info_buffer.raw");
            fgWrData2PC((void *)(_pucErrBuffer[u4VDecID][0]), 32*(2304/16), 7, "/mnt/sdcard1/LAE_error_buffer.raw");
        }
#endif
        vVDecAVCSetCoreState(u4VDecID, AVC_CORE_1_ID);
        printk("[INFO] AVC_CORE_1_ID input Window: 0x%08X\n", u4VDecReadAVCVLD(u4VDecID, RO_AVLD_BARL));
        vVDecAVCSetCoreState(u4VDecID, AVC_CORE_0_ID);
        printk("[INFO] AVC_CORE_0_ID input Window: 0x%08X\n", u4VDecReadAVCVLD(u4VDecID, RO_AVLD_BARL));

        // enable MCore decode
        vVDecWriteAVCMCORE_TOP(u4VDecID, 24*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 24*4) | 0x1);
        printk("AVCMCORE_TOP[24] = 0x%x\n", u4VDecReadAVCMCORE_TOP(u4VDecID, 24*4));
#if 0
        printk("[AVC] Before MCORE decode, LAE register dump\n");
        vVDecAVCSetCoreState(u4VDecID, AVC_LAE_0_ID);
        vVDEC_HAL_H264_VDec_DumpReg_LAE(u4VDecID);
        printk("[AVC] Before MCORE decode, Core0 register dump\n");
        vVDecAVCSetCoreState(u4VDecID, AVC_CORE_0_ID);
        vVDEC_HAL_H264_VDec_DumpReg_MCORE(u4VDecID);
        printk("[AVC] Before MCORE decode, Core1 register dump\n");
        vVDecAVCSetCoreState(u4VDecID, AVC_CORE_1_ID);
        vVDEC_HAL_H264_VDec_DumpReg_MCORE(u4VDecID);
        printk("[AVC] Before MCORE decode, MCORE_TOP register dump\n");
        vVDecAVCSetCoreState(u4VDecID, AVC_CORE_0_ID);
        vVDEC_HAL_H264_VDec_DumpReg_MCORE_TOP(u4VDecID);
#endif
        //Trigger MCore decode
        vVDecWriteAVCMCORE_TOP(u4VDecID, 25*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 25*4) | 0x1);
        printk("AVCMCORE_TOP[25] = 0x%x\n", u4VDecReadAVCMCORE_TOP(u4VDecID, 25*4));
...
{
    *pu4Mbx = u4VDecReadAVCMC(u4VDecID, RO_MC_MBX);
    *pu4Mby = u4VDecReadAVCMC(u4VDecID, RO_MC_MBY);
}
...
void vVDEC_HAL_H264_SetInterViewB0RefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_P_REF_PRM_T *prPRefPicListInfo)
{
    ...
    if (ucFld == FRAME)
    {
        // B_0
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#endif
...
void vVDEC_HAL_H264_SetInterViewB1RefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_P_REF_PRM_T *prPRefPicListInfo)
{
    ...
    if (ucFld == FRAME)
    {
        ...
        ucFBufIdx = prPRefPicListInfo->ucFBufIdx;
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), PHYSICAL((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteAVCMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#endif
...
{
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8555)
    vVDecAVCPowerDownHW(u4VDecID);
#endif
}
void vVDEC_HAL_H264_MCore_LAE_set_EC_register(UCHAR u4VDecID)
{
    vVDecWriteAVCVLD(u4VDecID, RW_AVLD_ERR_MASK, (~(AVLD_MB_END_CHK | AVLD_4BLOCKS_SKIP_CHK)));
}
int vVDEC_HAL_H264_Wait_LAE_decode_finished(UINT32 u4VDecID)
{
    UINT32 u4Cnt = 0;
    ...
    while (u4CntTimeChk < DEC_RETRY_NUM)
    {
        ...
        {
#ifdef IRQ_DISABLE
            if (u4VDEC_HAL_H264_VDec_ReadFinishFlag(u4VDecID))
#endif
            {
                _fgVDecComplete[u4VDecID] = TRUE;
                return 0;
            }
            u4CntTimeChk ++;
            u4Cnt = 0;
...
void vVDEC_HAL_H264_MCore_init(UCHAR u4VDecID, UINT32 u4LaeBufAddr, UINT32 u4ErrBufAddr)
{
    // MCORE_TOP Soft Reset 
    vVDecWriteAVCMCORE_TOP(u4VDecID, 7*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 7*4) | 0x1);
    vVDecWriteAVCMCORE_TOP(u4VDecID, 7*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 7*4) & ~0x1);

    // set addr for LAE Write
    vVDecWriteAVCMCORE_TOP(u4VDecID, 8*4, PHYSICAL(u4LaeBufAddr));
    vVDecWriteAVCMCORE_TOP(u4VDecID, 9*4, PHYSICAL(u4ErrBufAddr));
    
    // set addr for MCore read
    vVDecWriteAVCMCORE_TOP(u4VDecID, 16*4, PHYSICAL(u4LaeBufAddr));
    vVDecWriteAVCMCORE_TOP(u4VDecID, 17*4, PHYSICAL(u4ErrBufAddr));
    vVDecWriteAVCMCORE_TOP(u4VDecID, 18*4, 0x120); // 288, unit is 16 bytes
}
void vVDEC_HAL_H264_MCore_Enable(UCHAR u4VDecID, BOOL fgEnable)
{
    if (fgEnable)
    {
        ...
        u4RetVal = u4VDecReadAVCMCORE_TOP(u4VDecID, 12*4);
        vVDecWriteAVCMCORE_TOP(u4VDecID, 12*4, (u4RetVal | (1 << 0)));
        // turn on Multi-Core Read
        u4RetVal = u4VDecReadAVCMCORE_TOP(u4VDecID, 12*4);
        vVDecWriteAVCMCORE_TOP(u4VDecID, 12*4, (u4RetVal | (1 << 4)));       
        printk("smart mode read/write MCORE_TOP[12] read == 0x%x\n", u4VDecReadAVCMCORE_TOP(u4VDecID, 12*4));
    } else {
        vVDecWriteAVCMCORE_TOP(u4VDecID, 24*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 24*4) & ~0x1);
...
void VDec_DumpH264Reg(UCHAR ucMpvId)
{
    ...
    u4DumpIdx = 12;    //0x30
    u4Idx = 136; // 0x220
    pu4DumpArea[u4DumpIdx] = u4VDecReadAVCMC(ucMpvId, 4 * u4Idx);
    u4DumpIdx ++;
    u4Idx = 137;
    pu4DumpArea[u4DumpIdx] = u4VDecReadAVCMC(ucMpvId, 4 * u4Idx);
    u4DumpIdx ++;
    ...
    for (i = 0; i < 32; i++)
    {
        pu4DumpArea[u4DumpIdx] = u4VDecReadAVCMC(ucMpvId, RW_MC_P_LIST0  + 4 * i);
        u4DumpIdx ++;
    }
    ...
    for (i = 0; i < 32; i++)
    {
        pu4DumpArea[u4DumpIdx] = u4VDecReadAVCMC(ucMpvId, RW_MC_B_LIST0  + 4 * i);
        u4DumpIdx ++;
    }
    ...
    for (i = 0; i < 32; i++)
    {
        pu4DumpArea[u4DumpIdx] = u4VDecReadAVCMC(ucMpvId, RW_MC_B_LIST1  + 4 * i);
        u4DumpIdx ++;
    }
    ...
    for (j = 0; j < 8; j++)
    {
        if (j < 6)
        {
            ...
            {
                // add 16 for every list
                vVDecWriteAVCCOMVLD(ucMpvId, RW_VLD_SCL_ADDR, 0x100 + dwTemp + (i << 2));
                pu4DumpArea[u4DumpIdx] = u4VDecReadAVCCOMVLD(ucMpvId, RW_VLD_SCL_DATA);
                u4DumpIdx ++;
            }
        }
        else
...
UINT32 u4VDEC_HAL_H264_VDec_ReadFinishFlag(UINT32 u4VDecID)
{
    if (bMode_MCORE[u4VDecID]) {
    	if ( _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4CoreID == AVC_LAE_0_ID ){
	    	return ((u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) >> 24) & 0x1);
    	} else {
	    	UINT32 core0_finish = (u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) >> 16) & 0x1;
		    UINT32 core1_finish = (u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) >> 17) & 0x1;
    		return (core0_finish & core1_finish);
	    }
    } else {
        return u4VDecReadAVCVLD(u4VDecID, RO_AVLD_COMPLETE);
    }
}
UINT32 u4VDEC_HAL_H264_VDec_ClearInt(UINT32 u4VDecID)
{
    if (bMode_MCORE[u4VDecID]) {
        if ( _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4CoreID == AVC_LAE_0_ID ){
            vVDecWriteAVCMCORE_TOP(u4VDecID, 3*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) | 0x1 << 24);
        } else {
            vVDecWriteAVCMCORE_TOP(u4VDecID, 3*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) | 0x1 << 16);
            vVDecWriteAVCMCORE_TOP(u4VDecID, 3*4, u4VDecReadAVCMCORE_TOP(u4VDecID, 3*4) | 0x1 << 17);
        }
    }
}
void vVDEC_HAL_H264_VDec_DumpReg_LAE(UINT32 u4VDecID)
{
    UINT32 u4Idx;
    UINT32 u4RegValue;
    for (u4Idx = 33; u4Idx <= 255; u4Idx++) {
        u4RegValue = u4VDecReadAVCCOMVLD(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_VLD( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
	for (u4Idx = 0; u4Idx <= 135; u4Idx++) {
        u4RegValue = u4VDecReadAVCVLDTOP(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_VLD_TOP( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
	for (u4Idx = 33; u4Idx <= 141; u4Idx++) {
        u4RegValue = u4VDecReadAVCVLD(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_AVC_VLD( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
	for (u4Idx = 145; u4Idx <= 511; u4Idx++) {
        u4RegValue = u4VDecReadAVCVLD(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_AVC_VLD( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
    for (u4Idx = 0; u4Idx <= 255; u4Idx++) {
        u4RegValue = u4VDecReadAVCMV(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_AVC_MV( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
	for (u4Idx = 0; u4Idx <= 127; u4Idx++) {
        u4RegValue = u4VDecReadAVCMISC(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_MISC( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
}
void vVDEC_HAL_H264_VDec_DumpReg_MCORE_TOP(UINT32 u4VDecID)
{
    UINT32 regIdxList[11] = {3,7,8,9,12,14,16,17,18,24,25};
    UINT32 u4Idx;
    UINT32 u4RegValue;
#if 0
    int i;
    for (i = 0; i < sizeof(regIdxList)/sizeof(regIdxList[0]); i ++) {
        u4Idx = regIdxList[i];
        u4RegValue = u4VDecReadAVCMCORE_TOP(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_MCORE_TOP( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
#endif
    for (u4Idx = 0; u4Idx <= 76; u4Idx++) {
        u4RegValue = u4VDecReadAVCMCORE_TOP(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_MCORE_TOP( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
}
void vVDEC_HAL_H264_VDec_DumpReg_MCORE(UINT32 u4VDecID)
{
    UINT32 u4Idx;
    UINT32 u4RegValue;
    for (u4Idx = 33; u4Idx <= 255; u4Idx++) {
    ...
    for (u4Idx = 0; u4Idx <= 1023; u4Idx++) {
        u4RegValue = u4VDecReadAVCMC(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_MC( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
    for (u4Idx = 0; u4Idx <= 255; u4Idx++) {
        u4RegValue = u4VDecReadAVCMV(u4VDecID, 4 * u4Idx);
    ...
    for (u4Idx = 0; u4Idx <= 1023; u4Idx++) {
        u4RegValue = u4VDecReadAVCPP(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_PP( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
	for (u4Idx = 0; u4Idx <= 127; u4Idx++) {
        u4RegValue = u4VDecReadAVCMISC(u4VDecID, 4 * u4Idx);
    ...
    for (u4Idx = 33; u4Idx <= 255; u4Idx++) {
        u4RegValue = u4VDecReadAVCBS2(u4VDecID, 4 * u4Idx);
        printk("[AVC] RISCRead_BS2( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
}
...
void vVDEC_HAL_H264_VDec_ReadCheckSum(UINT32 u4VDecID, UINT32 *pu4CheckSum)
{
    ...
    u4Temp = 0;
    *pu4CheckSum = u4VDecReadAVCMC(u4VDecID, 0x5f4);
    pu4CheckSum ++;
    u4Temp ++;
    *pu4CheckSum = u4VDecReadAVCMC(u4VDecID, 0x5f8);
    pu4CheckSum ++;
    u4Temp ++;
    *pu4CheckSum = u4VDecReadAVCMC(u4VDecID, 0x608);
    pu4CheckSum ++;
    u4Temp ++;
    *pu4CheckSum = u4VDecReadAVCMC(u4VDecID, 0x60c);
    pu4CheckSum ++;
    u4Temp ++;
    ...
    {
        *pu4CheckSum = u4VDecReadAVCMC(u4VDecID, (u4Cnt << 2));
        pu4CheckSum ++;
        u4Temp ++;
    }
...
void vVDEC_HAL_H264_VDec_ReadCheckSum1(UINT32 u4VDecID)
{
    ...
    *pu4CheckSum = u4VDecReadAVCCOMVLD(u4VDecID, (44 << 2));
    pu4CheckSum ++;
    u4Temp ++;

    *pu4CheckSum = u4VDecReadAVCCOMVLD(u4VDecID, (45 << 2));
    pu4CheckSum ++;
    u4Temp ++;

    *pu4CheckSum = u4VDecReadAVCCOMVLD(u4VDecID, (46 << 2));
    pu4CheckSum ++;
    u4Temp ++;
    ...
    {
        *pu4CheckSum = u4VDecReadAVCCOMVLD(u4VDecID, (u4Cnt << 2));
        pu4CheckSum ++;
        u4Temp ++;
    }
