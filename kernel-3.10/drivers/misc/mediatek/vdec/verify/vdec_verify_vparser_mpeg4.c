...
UINT32 u4VParserMPEG4(UINT32 u4InstID, BOOL fgInquiry)
{
    ...
    _fgVerVopCoded0[u4InstID] = FALSE;
     ...
     //printk("u4VParserMPEG4 before vVDEC_HAL_MPEG_SetMPEG4Flag\n");
    vVDEC_HAL_MPEG_SetMPEG4Flag(u4InstID, TRUE);

    //printk("u4VParserMPEG4 before vVerifyGetUpParm\n");
    ...
    //printk("u4VParserMPEG4 before switch case\n");
    switch (_u4CodecVer[u4InstID])
    {
        ...
        case VDEC_S263:
            ...
            //printk("u4VParserMPEG4 before u4VDEC_HAL_MPEG_ShiftGetBitStream\n");
            ...
            //printk("u4VParserMPEG4 while loop  fgVPrsValidStartCode\n");
    ...
    //printk("u4VParserMPEG4 before fgInquiry\n");
    if (!fgInquiry)
    {
    ...
    //printk("u4VParserMPEG4 after fgInquiry\n");
    if (dwRetVal != 0)
    {
        return (dwRetVal);
    }

    //printk("u4VParserMPEG4 before vVerifyMpvSetSeqLayerDecPrm\n");
    vVerifyMpvSetSeqLayerDecPrm(u4InstID);

    //printk("u4VParserMPEG4 before vVerifyMp4SetDecPrm\n");
    vVerifyMp4SetDecPrm(u4InstID);

    //printk("u4VParserMPEG4 before u4BBufStart\n");
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.u4BBufStart = 0;

    //printk("u4VParserMPEG4 before return\n");
    return (0);
}
...
UINT32 u4ShVParser(UINT32 u4InstID, BOOL fgInquiry)
{
    ...
    //PANDA H263
    if (_u4CodecVer[u4InstID] == VDEC_H263 || _u4CodecVer[u4InstID] == VDEC_S263)
    {
        BYTE bPictureSize;
...
void vVerifyCalGmcMv(UINT32 u4InstID)
{
    ...
    long i0_pla, j0_pla;
    bool bPureIsoM4v; // replacing fgPureIsoM4v()


    if ((_u4CodecVer[u4InstID] == VDEC_MPEG4) || (_u4CodecVer[u4InstID] == VDEC_H263) || (_u4CodecVer[u4InstID] == VDEC_S263))
    {
        bPureIsoM4v   = TRUE;
    ...
    {
        printk("vVerifyMp4SetDecPrm unexpected format something wrong\n");
    }
...
UINT32 u4M4VParser(UINT32 u4InstID, BOOL fgInquiry)
{
    ...
    */
    ...
    else if ((_u4CodecVer[u4InstID] == VDEC_S263) && ((_u4Datain[u4InstID] & H263_VIDEO_START_MASK) == H263_VIDEO_START_MARKER)) // Sorenson
    {
    ...
    else if ((_u4CodecVer[u4InstID] != VDEC_S263) && ((_u4Datain[u4InstID] & SHORT_VIDEO_START_MASK) == SHORT_VIDEO_START_MARKER)) // H.263
    {
        if ((dwRetVal = u4ShVParser(u4InstID, fgInquiry)) != 0)
        {
            return (dwRetVal);
    ...
    if (fgMp4)
    {
        ...
        else
        {
                ...
                ((*pdwSC) <= VIDEO_OBJECT_START_CODE_MAX))
            {
                ...
                else if ((_u4CodecVer[u4InstID] == VDEC_S263) && ((dwDatain & H263_VIDEO_START_MASK) == H263_VIDEO_START_MARKER)) // Sorenson
                {
                    *pdwSC = dwDatain;
                    return TRUE;
                }
                else if ((_u4CodecVer[u4InstID] != VDEC_S263) && ((dwDatain & SHORT_VIDEO_START_MASK) == SHORT_VIDEO_START_MARKER))
                {
                    *pdwSC = dwDatain;
                    return TRUE;
                }
            ...
            else if ((_u4CodecVer[u4InstID] == VDEC_S263) && (((*pdwSC) & H263_VIDEO_START_MASK) == H263_VIDEO_START_MARKER)) // Sorenson
            {
                return TRUE;
            }
            else if (/*(_fgShortHeader[u4InstID])&&*/(_u4CodecVer[u4InstID] != VDEC_S263) && (((*pdwSC)& SHORT_VIDEO_START_MASK) == SHORT_VIDEO_START_MARKER))
