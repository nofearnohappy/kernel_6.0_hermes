...
#include "../hal/vdec_hw_h264.h" // vVDecAVCSetCoreState()
...
void vPrepareRefPiclist(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
    ...
    if (bMode_MCORE[u4InstID]) {
        if ( _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.u4CoreID == AVC_LAE_0_ID ){
#ifdef REDEC
...
void vVerifyVDecSetPicInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
    ...
    if ( !bMode_MCORE[u4InstID] || _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.u4CoreID != AVC_LAE_0_ID )
    {
...
void vPartitionDPB(UINT32 u4InstID)
{
    ...
    if (bMode_10bit[u4InstID]) {
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum = DPB_SZ / ((((_tVerMpvDecPrm[u4InstID].u4PicW * _tVerMpvDecPrm[u4InstID].u4PicH) * 5) >> 2) * 7 / 4);
    } else {
    ...
    if (bEnable_UFO[u4InstID]) {
        ...
        if (bMode_10bit[u4InstID]) {
        	u4PIC_SIZE_Y[u4InstID] = ((u4PIC_SIZE_Y[u4InstID] * 5) >> 2);
	        // u4UFO_LEN_SIZE_Y is the same as 8 bit
        }
        ...
        if (bMode_10bit[u4InstID]) {
            _tVerMpvDecPrm[u4InstID].u4DramPicSize = (((((_tVerMpvDecPrm[u4InstID].u4PicW + 63) >> 6) << 6)* 5) >> 2) * (((_tVerMpvDecPrm[u4InstID].u4PicH + 31) >> 5) << 5);
        } else {
        ...
        if (bMode_10bit[u4InstID]) {
    	    u4DramPicSize = (((((_tVerMpvDecPrm[u4InstID].u4PicW + 63) >> 6) << 6)* 5) >> 2) * (((_tVerMpvDecPrm[u4InstID].u4PicH + 31) >> 5) << 5);
        } else {
...
// Return      : None
// *********************************************************************
UINT32 vSearchRealPic(UINT32 u4InstID)
{
    UINT32 u4RetVal = AVC_PARSE_OK;
    UINT32 u4Temp;
    ...
    if (bMode_MCORE[u4InstID]) {
        vVDecAVCSetCoreState(u4InstID, AVC_LAE_0_ID);
    }
    do
    {
        ...
        printk("_u4CurrPicStartAddr[u4InstID] = %d\n", _u4CurrPicStartAddr[u4InstID]);
        ...
        switch (_ucNalUnitType[u4InstID])
        {
            ...
            case IDR_SLICE:
                ...
                if (bMode_MCORE[u4InstID]) {
                    vVDEC_HAL_H264_MCore_init(u4InstID, _pucLAEBuffer[u4InstID][0], _pucErrBuffer[u4InstID][0] );
                    vVDEC_HAL_H264_MCore_Enable(u4InstID, 1);
                    vSlice_layer_without_partition_nonIDR(u4InstID); //LAE parse/set registers complete
                    vVDecAVCSetCoreState(u4InstID, AVC_CORE_0_ID);
                    i4VDEC_HAL_H264_InitBarrelShifter(_u4BSID[u4InstID], u4InstID, &rH264BSInitPrm);
                    u4VDEC_HAL_H264_GetStartCode_8530(_u4BSID[u4InstID], u4InstID);
                    vSlice_layer_without_partition_nonIDR(u4InstID); //Core0/Core1 parse/set registers complete
                } else {
...
#if 0
        if (bMode_MCORE[u4InstID] && _ucNalUnitType[u4InstID] == SPS) {
            VDEC_INFO_H264_SPS_T *prSPS = &_rH264SPS[u4InstID][0];
            UINT32 u4PicW = (prSPS->u4PicWidthInMbsMinus1 + 1) << 4;
            UINT32 u4PicH = (2 - prSPS->fgFrameMbsOnlyFlag) * (prSPS->u4PicHeightInMapUnitsMinus1 + 1) << 4; //32x
            if (u4PicW >= 1920 && u4PicH >= 1080) {
                // continue decoding
                printk("AVC_MCORE H264 %s\n", _bFileStr1[u4InstID][1]);
                printk("AVC_MCORE FHD resolution W= %u H= %u\n", u4PicW, u4PicH);
            } else {
                // skip decoding
                printk("AVC_MCORE skip %s\n", _bFileStr1[u4InstID][1]);
                printk("AVC_MCORE less than 1080p resolution W= %u H= %u\n", u4PicW, u4PicH);
                u4RetVal = AVC_NOT_SUPPORT;
                break;
            }
        }
#endif
