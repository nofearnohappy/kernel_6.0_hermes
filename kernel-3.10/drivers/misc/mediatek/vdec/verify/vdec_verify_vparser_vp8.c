...
void vVDEC_Vp8DumpReg(void)
{
    ...
    for (i = 0; i < 30; i++)
    {
		...
		for (i = 33; i < 256; i++)
		{
			printk("VLD %d (0x%04x): 0x%08x\n", i, (i << 2), u4VDecReadVLD(0,i * 4));
			msleep(2);
		}
	
		printk("******* VLD2 Registers *******\n");
		msleep(2);
		for (i = 33; i < 256; i++)
		{
			printk("VLD2 %d (0x%04x): 0x%08x\n", i, (i << 2), u4VDecReadVP8VLD2(0,i * 4));
			msleep(2);
		}
		...
		for (i = 0; i < 256; i++)
		{
			printk("MV %d (0x%04x): 0x%08x\n", i, (i << 2), u4VDecReadVP8MV(0, i * 4));
			msleep(2);
		}
		...
		i = 135;
		msleep(2);
		printk("MISC_135 %d (0x%04x): 0x%08x\n", i, (i << 2), u4VDecReadMISC(0, i * 4));
		msleep(2);
		printk("MISC_135 %d (0x%04x): 0x%08x\n", i, (i << 2), u4VDecReadMISC(0, i * 4));
		msleep(2);
		...
		i = 136;
		msleep(2);
		printk("MISC_136 %d (0x%04x): 0x%08x\n", i, (i << 2), u4VDecReadMISC(0, i * 4));
		msleep(2);
		printk("MISC_136 %d (0x%04x): 0x%08x\n", i, (i << 2), u4VDecReadMISC(0, i * 4));
		msleep(2);
}
...
void vVP8_CheckCRCResult(UINT32 u4InstID)//, UINT32 u4DecFrameCnt, UINT32 u4CRCResBuf)
{
    ...
    //REG_9: c_crc_checksum[127:96]
		...
		if ((_u4FileCnt[u4InstID] + 1) == VDEC_STOP_FRAME_INDEX)
		{
    ...
    {
#ifdef VDEC_BREAK_EN
	if (((_u4FileCnt[u4InstID] + 1) == VDEC_BREAK_FRAME_INDEX))
	{
		sprintf(strMessage, " VDEC break on this frame ==> Skip CRC check [%d] \n", _u4FileCnt[u4InstID]);
...
BOOL fgIsVP8VDecComplete(UINT32 u4InstID)
{
    ...
    if (_fgVDecComplete[u4InstID])
    {
        ...
        if((u4VDEC_HAL_VP8_VDec_ReadFinishFlag(u4InstID))&&((u4MbX+1)>=((prVDecVp8FrmHdr->u4Width>>4)&0x1FF))&&((u4MbY+1)>=((prVDecVp8FrmHdr->u4Height>>4)&0x1FF)))
        {
            return TRUE;
        }
        else
...
UINT32 vVerVP8DecEndProc_MB_ROW_START(UINT32 u4InstID)
{
    ...
    while (u4CntTimeChk < DEC_RETRY_NUM)
    {
        ...
        if ((u4Cnt & 0x3f) == 0x3f)
        {
            ...
            else if(((u4VDecReadVLDTOP(u4InstID, 0xB4))&0x1) == 0x1)
            {
                _fgVDecComplete[u4InstID] = TRUE;
