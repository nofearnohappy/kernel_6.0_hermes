...
UINT32 vHEVCVerifyVDecSetPicInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
    ...
    if (bMode_MCORE[u4InstID]){
        if ( _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.u4CoreID == HEVC_LAE_0_ID ){
            ...
            vVDEC_HAL_H265_MCore_LAE_set_EC_register(u4InstID);
    ...
    if (ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->rSPS_PTL.generalPTL.u4ProfileIdc != 3 &&
        ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->rSPS_PTL.generalPTL.bProfileCompatibilityFlag[3] != 1)
    {
        if(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicHeightInLumaSamples > HEVC_MAX_HEIGHT ||
           ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicWidthInLumaSamples  > HEVC_MAX_WIDTH)
        {
            printk("[ERROR] Resolution over spec: %d x %d !!!!\n",ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicWidthInLumaSamples,ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicHeightInLumaSamples);
            _u4VerBitCount[u4InstID] = 0xFFFFFFFF;
            _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bNoDecode = 1;
            return SET_REG_SYNTAX_ERROR;    //Check resotuion support
        }
    }
    return PARSE_OK;
...
UINT32 vHEVCSearchRealPic(UINT32 u4InstID)
{
    ...
    if (bMode_MCORE[u4InstID]){
         vVDecHEVCSetCoreState(u4InstID, HEVC_LAE_0_ID);
    }
        ...
        switch (_ucNalUnitType[u4InstID])
        {
            ...
            case NAL_UNIT_CODED_SLICE_TFD:
                if (_u4SkipPicNum[u4InstID] >= _u4StartCompPicNum[u4InstID])
                {
                    ...
                    if (bMode_MCORE[u4InstID]){
                        vVDEC_HAL_H265_MCore_init(u4InstID, _pucLAEBuffer[u4InstID][0], _pucErrBuffer[u4InstID][0] );
                        vVDEC_HAL_H265_MCore_Enable (u4InstID, 1);
                        u4RetVal = vHEVCParseSliceHeader(u4InstID); //LAE parse/set registers complete

                        vVDecHEVCSetCoreState(u4InstID, HEVC_CORE_0_ID);

                        i4VDEC_HAL_H265_InitVDecHW(u4InstID);
                        i4VDEC_HAL_H265_InitBarrelShifter(_u4BSID[u4InstID], u4InstID, &rH265BSInitPrm);
                        u4Temp = u4VDEC_HAL_H265_GetStartCode_PicStart(_u4BSID[u4InstID], u4InstID) & 0xffff;
                        u4RetVal = vHEVCParseSliceHeader(u4InstID); //Core0/Core1 parse/set registers complete
...
UINT32 vHEVCParseSliceHeader(UINT32 u4InstID)
{
    ...
    if (0 == pH265_Slice_Hdr_DataInst->bDependentSliceSegmentFlag)
    {
        ...
        if (pH265_Slice_Hdr_DataInst->bFirstSliceSegmentInPic)
        {
            ...
            if (bMode_MCORE[u4InstID]){
                //not clear PicInfo for Core0/Core1
                if ( _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.u4CoreID == HEVC_LAE_0_ID){
                    vHEVCVerifyInitPicInfo(u4InstID);
                }
            } else {
