...
#include "s5k3m2trulymipi_Sensor.h"

#define PFX "S5K3M2TRULY_camera_sensor"
#define LOG_1 LOG_INF("S5K3M2TRULY,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3M2TRULY_SENSOR_ID,
...
struct S5K3m2truly_MIPI_otp_struct {
...
void S5K3m2truly_MIPI_read_otp_wb(struct S5K3m2truly_MIPI_otp_struct *otp)
{
    ...
    LOG_INF("[s5k3m2truly] [S5K3m2truly_MIPI_read_otp_wb] FLG=0x%x\n",FLG); 
   	mid_id  = read_3m2_eeprom_reg(0x0001); 
   
    LOG_INF("[s5k3m2truly] [S5K3m2truly_MIPI_read_otp_wb] mid_id=0x%x\n",mid_id); 
  	//if(FLG==0)
		//S5K3m2trulyDB("No OTP Data or OTP data is invalid");
   	//else
   	{
		golden_R = read_3m2_eeprom_reg(0x0016);           	 // read Golden value
		golden_Gr= read_3m2_eeprom_reg(0x0017);              // read Golden value
 		golden_Gb= read_3m2_eeprom_reg(0x0018);              // read Golden value
		golden_B = read_3m2_eeprom_reg(0x0019);              // read Golden value
  		LOG_INF("[S5K3m2truly] [S5K3m2truly_MIPI_read_otp_wb] golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);
  		//write_cmos_sensor(0x3a02, 0x00); //page set  
  		//write_cmos_sensor(0x3a00, 0x01); //otp enable read
  		current_R =  read_3m2_eeprom_reg(0x0012);
  		current_Gr = read_3m2_eeprom_reg(0x0013);
  		current_Gb = read_3m2_eeprom_reg(0x0014);
  		current_B =  read_3m2_eeprom_reg(0x0015); 
  		LOG_INF("[S5K3m2truly] [S5K3m2truly_MIPI_read_otp_wb] current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
   		...
   		LOG_INF("[S5K3m2truly] [S5K3m2truly_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", otp->r_ratio, otp->b_ratio);      
   	}
}



void S5K3m2truly_MIPI_algorithm_otp_wb1(struct S5K3m2truly_MIPI_otp_struct *otp)
{
   ...
   LOG_INF("[S5K3m2truly] [S5K3m2truly_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN,G_GAIN);    
}

void S5K3m2truly_MIPI_write_otp_wb(struct S5K3m2truly_MIPI_otp_struct *otp)
{
...
void S5K3m2truly_MIPI_update_wb_register_from_otp(void)
{
   struct S5K3m2truly_MIPI_otp_struct current_otp;
   
   S5K3m2truly_MIPI_read_otp_wb(&current_otp);
   S5K3m2truly_MIPI_algorithm_otp_wb1(&current_otp);
   S5K3m2truly_MIPI_write_otp_wb(&current_otp);
}
...
static void pip_capture_setting(void)
{
	...
	// e bruce, 2014/12/01
	LOG_INF( "S5K3M2truly PIP setting Enter!"); 
	write_cmos_sensor(0x6028,0x4000);
	write_cmos_sensor(0x32CA,0x022B);
	write_cmos_sensor(0x32D6,0x022B);
	write_cmos_sensor(0x0344,0x0010);//bpc off, no matter mirrir flip, PV & cap FOV the same
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
            ...
            //module_id = S5K3m2_MIPI_read_otp_wb(&current_otp);
...
static kal_uint32 open(void)
{
	...
	// Stream On
  	write_cmos_sensor(0x602A, 0x0100);
  	write_cmos_sensor_8(0x6F12, 0x01);
    S5K3m2truly_MIPI_update_wb_register_from_otp();
...
UINT32 S5K3M2TRULY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
