...
static u8 s5k3m2_eeprom_lsc_data[2048]={0};//liuying 20150310 add
...
static bool selective_read_byte(u32 addr, BYTE* data,u16 i2c_id)
{
    ...
    printk("selective_read_byte\n");
    kdSetI2CSpeed(EEPROM_I2C_SPEED);

    if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, i2c_id)) {
        printk("fail selective_read_byte addr =0x%x data = 0x%x", addr, *data);
        return false;
    }   
...
static int selective_read_region(u32 addr, BYTE* data,u16 i2c_id,u32 size)
{
    ...
    printk("selective_read_region addr =%x size %d data read = %d\n", addr,size, ret);
    return ret;
}
...
static bool _read_3m2_sunny_eeprom(kal_uint16 addr, BYTE* data, kal_uint32 size ){
	int i = 0;
	int offset = addr;
    size = size + 2;
	for(i = 0; i < size; i++) {
      if((offset != 0x0981) && (offset != 0x0982))  // invalid bit
      {
		if(!selective_read_eeprom(offset, &data[i])){
			return false;
		}
...
bool read_3m2_sunny_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size){
	...
	if(!get_done || last_size != size || last_offset != addr) {
		if(!_read_3m2_sunny_eeprom(addr, s5k3m2_eeprom_data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			return false;
...
static long s5ks5k3m2_otp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	printk("[CAMERA SENSOR] S5K3M2_OTP_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
#endif
{
    ...
    else
    {
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                printk("[S24CAM_CAL] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
			...
			printk("[S24CAM_CAL] After read Working buffer data  0x%x \n", *pu1Params);
#ifdef CAM_CALGETDLT_DEBUG
            do_gettimeofday(&ktv2);
            if(ktv2.tv_sec > ktv1.tv_sec)
            {
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        printk("[S24CAM_CAL] to user length %d \n", ptempbuf->u4Length);
        printk("[S24CAM_CAL] to user  Working buffer address 0x%p \n", pu1Params);
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pu1Params , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pu1Params);
            printk("[S24CAM_CAL] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        printk("[S24CAM_CAL] Allocate device no failed\n");
    ...
    {
        printk("[S24CAM_CAL] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        printk("[S24CAM_CAL] Allocate mem for kobject failed\n");
    ...
    {
        printk("[S24CAM_CAL] Attatch file operation failed\n");
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    printk("[CAM_CAL] g_pstI2Cclient->addr = 0x%x \n",g_pstI2Cclient->addr);
    //Register char driver
    i4RetValue = RegisterCAM_CALCharDrv();

    if(i4RetValue){
        printk("[S24CAM_CAL] register char device failed!\n");
        return i4RetValue;
    }


    printk("[S24CAM_CAL] Attached!! \n");
    return 0;
}
...
static int __init CAM_CAL_i2C_init(void)
{
    ...
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        printk("failed to register S24CAM_CAL driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        printk("failed to register S24CAM_CAL driver, 2nd time\n");
        return -ENODEV;
    }
