...
static int split_busy(DISP_MODULE_ENUM module)
{
	int idx = split_index(module);
	unsigned int state =
	    DISP_REG_GET_FIELD(DEBUG_FLD_SPLIT_FSM, DISP_REG_SPLIT_DEBUG + DISP_SPLIT_INDEX_OFFSET * idx);
	return state & 0x4;
}
...
static int split_dump_regs(DISP_MODULE_ENUM module)
{
	...
	unsigned int s_enable = DISP_REG_GET(DISP_REG_SPLIT_ENABLE + DISP_SPLIT_INDEX_OFFSET * idx);
	unsigned int s_sw_rst = DISP_REG_GET(DISP_REG_SPLIT_SW_RESET + DISP_SPLIT_INDEX_OFFSET * idx);
	unsigned int s_debug = DISP_REG_GET(DISP_REG_SPLIT_DEBUG + DISP_SPLIT_INDEX_OFFSET * idx);

	pr_debug("(0x000)S_ENABLE     =0x%x\n", s_enable);
	pr_debug("(0x004)S_SW_RST     =0x%x\n", s_sw_rst);
	pr_debug("(0x008)S_DEBUG      =0x%x\n", s_debug);
	return 0;
}
...
static int split_dump_analysis(DISP_MODULE_ENUM module)
{
	...
	unsigned int pixel =
	    DISP_REG_GET_FIELD(DEBUG_FLD_IN_PIXEL_CNT, DISP_REG_SPLIT_DEBUG + DISP_SPLIT_INDEX_OFFSET * idx);
	unsigned int state =
	    DISP_REG_GET_FIELD(DEBUG_FLD_SPLIT_FSM, DISP_REG_SPLIT_DEBUG + DISP_SPLIT_INDEX_OFFSET * idx);
