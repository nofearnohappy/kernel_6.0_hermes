...
int rdma_enable_irq(DISP_MODULE_ENUM module, void *handle, DDP_IRQ_LEVEL irq_level)
{
	...
	case DDP_IRQ_LEVEL_ERROR:
		DISP_REG_SET(handle, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_INT_ENABLE, 0x18);
		break;
...
int rdma_reset(DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET_FIELD(handle, GLOBAL_CON_FLD_SOFT_RESET, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_GLOBAL_CON,
			   1);
	while ((DISP_REG_GET(DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_GLOBAL_CON) & 0x700) == 0x100) {
		...
		if (delay_cnt > 10000) {
			...
			DDPERR("rdma%d_reset timeout, stage 1! DISP_REG_RDMA_GLOBAL_CON=0x%x\n", idx,
			       DISP_REG_GET(DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_GLOBAL_CON));
			break;
		}
	}
	DISP_REG_SET_FIELD(handle, GLOBAL_CON_FLD_SOFT_RESET, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_GLOBAL_CON,
		    ...
		    unsigned int frame_rate, void *handle)
{
	...
	/* write ultra_low_level, ultra_high_ofs, pre_ultra_low_ofs, pre_ultra_high_ofs into register
	...
	DISP_REG_SET_FIELD(handle, FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD,
			   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_FIFO_CON, fifo_valid_size);
...
static int rdma_config(DISP_MODULE_ENUM module,
		       ...
		       unsigned pitch,
		       unsigned width, unsigned height, unsigned ufoe_enable, DISP_BUFFER_TYPE sec, void *handle)
{
	...
	if (input_is_yuv == 1 && output_is_yuv == 0) {
		...
		DISP_REG_SET_FIELD(handle, SIZE_CON_0_FLD_MATRIX_ENABLE,
				   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_0, 0);
		DISP_REG_SET_FIELD(handle, SIZE_CON_0_FLD_MATRIX_INT_MTX_SEL,
				   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_0, 0);
	}
	...
	DISP_REG_SET_FIELD(handle, GLOBAL_CON_FLD_MODE_SEL, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_GLOBAL_CON,
			   mode);
	...
	DISP_REG_SET_FIELD(handle, MEM_CON_FLD_MEM_MODE_INPUT_FORMAT,
			   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_MEM_CON,
			   ((mode == RDMA_MODE_DIRECT_LINK) ? 0 : input_format_reg & 0xf));
	DISP_REG_SET_FIELD(handle, MEM_CON_FLD_MEM_MODE_INPUT_SWAP, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_MEM_CON,
	...
	if (sec != DISP_SECURE_BUFFER) {
		...
		cmdqRecWriteSecure(handle,
				   disp_addr_convert(DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_MEM_START_ADDR),
				   CMDQ_SAM_H_2_MVA, address, 0, size, m4u_port);
		/* DISP_REG_SET(handle,
			idx*DISP_RDMA_INDEX_OFFSET+DISP_REG_RDMA_MEM_START_ADDR, address-0xbc000000+0x8c00000); */
	}
	...
	DISP_REG_SET_FIELD(handle, SIZE_CON_0_FLD_OUTPUT_FRAME_WIDTH,
			   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_0, width);
	DISP_REG_SET_FIELD(handle, SIZE_CON_1_FLD_OUTPUT_FRAME_HEIGHT,
			   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_1, height);
	...
	if (ufoe_enable == 0) {
		DISP_REG_SET_FIELD(handle, FIFO_CON_FLD_FIFO_UNDERFLOW_EN,
				   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_FIFO_CON, 1);
		/* DISP_REG_SET_FIELD(
			handle,
			FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD,
			DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_FIFO_CON,
			0x100); */
	} else {
		DISP_REG_SET_FIELD(handle, FIFO_CON_FLD_FIFO_UNDERFLOW_EN,
				   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_FIFO_CON, 1);
		...
		DISP_REG_SET_FIELD(handle, FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD,
				   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_FIFO_CON, 496);
		/* DISP_REG_SET_FIELD(
			handle,
			FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD,
			DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_FIFO_CON,
...
int rdma_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_DYNAMIC_SWITCH_UNDERFLOW_EN)) {
		if (state == CMDQ_AFTER_STREAM_EOF) {
			DISP_REG_SET_FIELD(cmdq_trigger_handle, FIFO_CON_FLD_FIFO_UNDERFLOW_EN,
					   DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_FIFO_CON, 0);
...
void rdma_dump_reg(DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("RDMA:0x000=0x%08x,0x004=0x%08x,0x010=0x%08x,0x014=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON + off_st), DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 + off_st));
	...
	DDPDUMP("RDMA:0x018=0x%08x,0x01c=0x%08x,0x024=0x%08x,0xf00=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_RDMA_MEM_CON + off_st), DISP_REG_GET(DISP_REG_RDMA_MEM_START_ADDR + off_st));
	...
	DDPDUMP("RDMA:0x040=0x%08x,0x044=0x%08x,0x078=0x%08x,0x07c=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_RDMA_PRE_ADD_0 + off_st), DISP_REG_GET(DISP_REG_RDMA_PRE_ADD_1 + off_st));
	...
	DDPDUMP("RDMA:0x080=0x%08x,0x084=0x%08x,0x088=0x%08x,0x08c=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_RDMA_POST_ADD_1 + off_st), DISP_REG_GET(DISP_REG_RDMA_POST_ADD_2 + off_st));
...
void rdma_get_info(int idx, RDMA_BASIC_STRUCT *info)
{
	...
	p->src_h = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1 + DISP_RDMA_INDEX_OFFSET(idx)) & 0xfffff,
	    p->bpp =
	    rdma_input_format_bpp((DISP_REG_GET(DISP_REG_RDMA_MEM_CON + DISP_RDMA_INDEX_OFFSET(idx)) >> 4) & 0xf);
	return;
}
	...
	if (mode == RDMA_MODE_MEMORY) {
		if (security == DISP_SECURE_BUFFER) {
			...
			if (rdma_is_sec[rdma_idx] == 0)
				DDPDEBUG_D("[SVP] switch rdma%d to sec\n", rdma_idx);
			...
			if (rdma_is_sec[rdma_idx]) {
				...
				DDPDEBUG_D("[SVP] switch rdma%d to nonsec done\n", rdma_idx);
			}
