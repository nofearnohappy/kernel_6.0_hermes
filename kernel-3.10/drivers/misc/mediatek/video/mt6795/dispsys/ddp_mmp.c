		       ...
		       unsigned int session)
{
	...
	if (session == 1)
		...
		MMProfileLogEx(DDP_MMP_Events.Extd_layer_dump_parent, MMProfileFlagStart, pLayer->layer,
			       pLayer->layer_en);
	...
	if (pLayer->layer_en) {
		...
		if (!raw) {
			...
			if (m4u_mva_map_kernel
			    (pLayer->addr, Bitmap.data_size, (unsigned long *)&Bitmap.pData, &Bitmap.data_size) == 0) {
				if (session == 1)
					MMProfileLogMetaBitmap(DDP_MMP_Events.layer_dump[pLayer->layer],
			...
			if (m4u_mva_map_kernel(pLayer->addr, meta.size,
				...
				if (session == 1)
					MMProfileLogMeta(DDP_MMP_Events.layer_dump[pLayer->layer], MMProfileFlagPulse,
							 &meta);
				else if (session == 2)
			...
			unsigned int down_sample_y)
{
	...
	if (!raw) {
		...
		if (m4u_mva_map_kernel(wdma_layer->dstAddress, meta.size, (unsigned long *)&meta.pData, &meta.size) ==
		    0) {
