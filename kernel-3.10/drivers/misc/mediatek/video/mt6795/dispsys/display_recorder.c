...
void dprec_logger_trigger(unsigned int type_logsrc, unsigned int val1, unsigned int val2)
{
	...
	if (source == DPREC_LOGGER_DSI_EXT_TE) {
		/* DISPMSG("count=%d, time=%llu, period=%llu, max=%llu,min=%llu,start=%llu,trigger=%llu\n",
			l->count, time, l->period_frame, l->period_max_frame,
...
void dprec_logger_start(unsigned int type_logsrc, unsigned int val1, unsigned int val2)
{
	...
	/*if (source == DPREC_LOGGER_RDMA0_TRANSFER)*/
		...
		(unsigned long long)nsec_high(l->ts_trigger), (unsigned long)nsec_low(l->ts_trigger)); */
...
void dprec_start(dprec_logger_event *event, unsigned int val1, unsigned int val2)
{
	if (event) {
		...
		if (event->level & DPREC_LOGGER_LEVEL_UART_LOG)
			pr_info("DISP/%s start,0x%08x,0x%08x\n", event->name, val1, val2);
#ifdef CONFIG_TRACING
...
int dprec_logger_get_result_value(DPREC_LOGGER_ENUM source, unsigned long long *pcount, unsigned long long *pavg,
				  unsigned long long *pfps, unsigned long long *pmax, unsigned long long *pmin)
{
	unsigned long flags = 0;
	spin_lock_irqsave(&gdprec_logger_spinlock, flags);
	int len = 0;
...
void dprec_reg_op(void *cmdq, unsigned int reg, unsigned int val, unsigned int mask)
{
	...
	/* printk("[DISP/REG]handle=0x%lx, [%s]0x%x=0x%lx&0x%x\n",
		(unsigned long)cmdq, _find_module_by_reg_addr(dprec_addr_convert(reg)),
		(unsigned int)reg, (unsigned long)val, (unsigned int)mask); */
	...
	if (cmdq) {
		...
		if (!in_interrupt()) {
			if (mask)
				...
				DISPPR_HWOP("%s/%08x=%08x\n", _find_module_by_reg_addr(dprec_addr_convert(reg)), reg,
					    val);
		}
	}
	...
	/*if (_control.cmm_dump)*/
		/* pr_info("[CMM]D.S SD:0x%08x %\LE %\LONG 0x%08x; write %s\n",
			(_control.cmm_dump_use_va) ? reg : (reg&0x1fffffff), mask ? (val|mask) : val,
...
{
	analysize_length +=
	    scnprintf(dprec_string_buffer_analysize + analysize_length, dprec_dump_max_length - analysize_length,
		      string);
}
