...
char *ddp_get_scenario_name(DDP_SCENARIO_ENUM scenario)
{
	...
	case DDP_SCENARIO_PRIMARY_RDMA_UFOE_MEMOUT:
		return "primary_rdma_ufoe_memout";
	...
	case DDP_SCENARIO_PRIMARY_ALL_UFOE_WDMA:
		return "primary_all_UFOE_WDMA";
	case DDP_SCENARIO_PRIMARY_OD_1TO2:
		return "primary_od_1to2";
...
static void ddp_check_mutex_l(int mutex_id, int *module_list, DDP_MODE ddp_mode)
{
	...
	if ((kal_uint32) expect_sof != real_sof) {
		DDPDUMP("error:mutex %d sof error: expect %s, real %s\n", mutex_id,
			ddp_get_mutex_sof_name(expect_sof), ddp_get_mutex_sof_name((MUTEX_SOF) real_sof));
	}
}
