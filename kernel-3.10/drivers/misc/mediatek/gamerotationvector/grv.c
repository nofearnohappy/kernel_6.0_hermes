...
static struct grv_context *grv_context_obj = NULL;
...
static void grv_work_func(struct work_struct *work)
{
	...
	grv_data_report(cxt->drv_data.grv_data.values[0],
		...
		cxt->drv_data.grv_data.values[3],
		cxt->drv_data.grv_data.status);
...
static ssize_t grv_show_enable_nodata(struct device* dev, 
...
/*----------------------------------------------------------------------------*/
static ssize_t grv_show_active(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	struct grv_context *cxt = NULL;
	int div;
...
static ssize_t grv_show_delay(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
    int len = 0;
	GRV_LOG(" not support now\n");
...
static ssize_t grv_show_sensordevnum(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	struct grv_context *cxt = NULL;
	char *devname = NULL;
...
static ssize_t grv_show_batch(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 0); 
}
...
static ssize_t grv_show_flush(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 0); 
}
...
int grv_register_control_path(struct grv_control_path *ctl)
{
	...
	err = sysfs_create_group(&grv_context_obj->mdev.this_device->kobj,
			&grv_attribute_group);
	if (err < 0)
	{
...
static int grv_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&grv_context_obj->idev->dev.kobj,
				&grv_attribute_group);
	
	if((err = misc_deregister(&grv_context_obj->mdev)))
	{
